  ...
  <div class="Self-container">
    <div class="self-left">
      <div>
        <div class="selfTitle">
          我的歌单
          <span class="iconfont icon-jia" style="font-size: 22px; float: right; margin-right: 10px; cursor: pointer" @click="createSongList()"></span>
        </div>
      ...
      <div>
        ...
        <ul v-if="importUserInfo.length" class="addList">
          <li v-for="item in importUserInfo" :key="item.id" class="addItem" @click.prevent="goImportList(item)">
    ...
    <div v-if="importUserInfo.length || prevPlayInfo.name" class="self-right">
      <div :class="titleActive ? 'titleActive' : 'infoBox'">
        <div v-if="prevPlayInfo.coverImgUrl" class="songListCover">
          <img :src="prevPlayInfo.coverImgUrl" alt srcset />
        </div>
        <div class="songListInfoBox">
          <span class="songListName">{{ prevPlayInfo.name }}</span>
          <span class="songListDescription">{{ prevPlayInfo.description }}</span>
          <div v-if="!titleActive" class="songListSum" style="margin-left: 20px">
            <span calss="number" style="margin-right: 5px"
              >共<i style="fontweight: 700"> {{ prevPlayInfo.trackCount }} </i>首</span
            >
            <span v-if="prevPlayInfo.playCount" calss="number" style="margin-right: 5px">
              共播放
              <i style="font-weight: 700">{{
                prevPlayInfo.playCount.toString().length > 4 ? `${prevPlayInfo.playCount.toString().slice(0, -4)}万` : prevPlayInfo.playCount
              }}</i>
              次
            </span>
          </div>
      ...
      <div class="songList">
        <ul>
          ...
          <li v-for="(item, index) in prevPlayList" :key="index" :class="{ bgEven: (index + 1) % 2 == 0 ? true : false }">
...
const prevPlayInfo = computed(() => musicStore.prevPlayInfo);
...
const importList = async () => {
  if (!importUserInfo.value.length) {
    ...
    const { code, playlist } = await request.get("/user/playlist?uid=" + uid, {});
    if (code === 200) {
      musicStore.setImportUserInfo(playlist);
      localStorage.setItem("WYSELFLIST", JSON.stringify(playlist));
    }
  }
};

const goImportList = async (item: object) => {
  musicStore.setPrevPlayInfo(item);
  const { code, songs } = await request.get(`/playlist/track/all?id=${item.id}`); // &limit=10&offset=1 limit : 限制获取歌曲的数量，默认值为当前歌单的歌曲数量
  if (code === 200) {
    musicStore.setPrevPlayList(songs); // data.playlist
  }
};

const goSongList = (songListName: string) => {
  JSON.parse(localStorage.getItem("addList") as string).forEach((ele: any) => {
    if (ele.name == songListName) {
      ele.trackCount = ele.tracks.length;
      musicStore.setPrevPlayList(ele);
    }
  });
};
const goSongDetails = (songId: number) => {
  prevPlayList.value.forEach((song: any) => {
    if (song.id == songId) {
      musicStore.setPrevPlaySong(song);
    }
  });
  router.push({
    name: "songDetails",
...
const handleCommand = (command) => {
  prevPlayInfo.value.tracks.forEach((item, n) => {
